  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "CalculateCardCLKD()\n"));
  ...
  *ClockFrequencySelect = ((MMC_REFERENCE_CLK/Frequency) + 1);
  ...
  DEBUG ((EFI_D_INFO, "mMaxDataTransferRate: 0x%x, Frequency: %d KHz, ClockFrequencySelect: %x\n", mMaxDataTransferRate, Frequency/1000, *ClockFrequencySelect));
}
...
{
  DEBUG ((EFI_D_INFO, "UpdateMMCHSClkFrequency()\n"));
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "InitializeMMCHS()\n"));
  ...
  )
{
  ...
  if (RetryCount == MAX_RETRY_COUNT) {
    DEBUG ((EFI_D_INFO, "MMCSendCommand: Timeout\n"));
    CpuDeadLoop();
    return EFI_TIMEOUT;
  }
  ...
  )
{
  ...
  switch(State) {
    ...
    case MmcHwInitializationState:
      ...
      DEBUG ((EFI_D_INFO, "MMCHwInitializationState()\n"));
      Status = InitializeMMCHS ();
      if (EFI_ERROR(Status)) {
        DEBUG ((EFI_D_INFO, "Initialize MMC host controller fails. Status: %x\n", Status));
        return Status;
      }
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "MMCReadBlockData(LBA: 0x%x, Length: 0x%x, Buffer: 0x%x)\n", Lba, Length, Buffer));
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "MMCInitialize()\n"));
